...
import { HEVCDecoderConfigurationRecord } from './HEVCDecoderConfig';
...
export class MediaStreamReceiver {
  ...
  private playDecodedAudio = (audioData: AudioData): void => {
    try {
      ...
      for (let ch = 0; ch < numberOfChannels; ch++) {
        const channelData = tempBuffer.subarray(
          ch * numberOfFrames,
          (ch + 1) * numberOfFrames,
        );
  ...
  private newCodecFromDescription = (
    buffer: ArrayBuffer,
    receivedCodec: string,
  ): string => {
    // Chỉ check nếu codec là HEVC (hvc1 hoặc hev1)
    if (
      !receivedCodec.toLowerCase().includes('hvc') &&
      !receivedCodec.toLowerCase().includes('hev')
    )
      return '';
            ...
            try {
              ...
              if (this.videoDecoder && this.videoDecoder.state !== 'closed') {
                ...
                if (support.supported) {
                  ...
                  console.error(
                    '❌ Browser does not support this video config:',
                    decoderConfig,
                  );
                }
              }
          ...
          case FRAME_TYPE.ORIENTATION: {
            ...
            if (
              ...
              try {
                ...
                console.log(
                  '🔄 Reconfigured rotation to',
                  orientation,
                  '- Waiting for next KeyFrame',
                );
              } catch (configErr) {
                console.error(
                  'Error reconfiguring VideoDecoder with new orientation:',
                  configErr,
                );
              }
            }
